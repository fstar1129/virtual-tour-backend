...
class DistanceCalculator
{
    ...
    public function __construct($tour)
    {
        ...
        if ($this->tour->isAdventure()) {
            $this->possible_paths = [];
            $this->current_path = [];
            $this->firstStop = $this->getFirstStop()->id;
            $this->lastStop = $this->getLastStop()->id;
            $this->nextStopList = [];
            foreach ($this->stops as $stop) {
                ...
                if (empty($nextStops)) {
                    // last stop
                    $this->nextStopList[$stop->id] = [];
                    continue;
                }
    
                $this->nextStopList[$stop->id] = $nextStops->pluck('id')->toArray();
            }
        }
    }
     ...
     * @throws UntraceableTourException
     */
    public function goToNextStop($current_stop) {
        array_push($this->current_path, $current_stop);
        if ($current_stop == $this->lastStop) {
            array_push($this->possible_paths, $this->current_path);
            array_pop($this->current_path);
        } else {
            foreach ($this->nextStopList[$current_stop] as $nextStop) {
                if ( !in_array($nextStop, $this->current_path) ) {
                    $this->goToNextStop($nextStop);
                }
            }
            array_pop($this->current_path);
        }
    ...
    /**
     * Get an array of all the valid possible paths for the tour.
     *
     * @return \Illuminate\Support\Collection
     * @throws UntraceableTourException
     */
    public function getPossiblePaths() {
        $this->goToNextStop($this->firstStop);
        return collect($this->possible_paths)->values();
    }
    ...
    public function getDistanceBetweenStops($stop1, $stop2)
    {
        ...
        if (! $path || $path->count() == 0) {
            ...
            // return the distance between these two stops.
            $firstPoint = $stop1->location;
            $lastPoint = $stop2->location;
            $distance = $this->getDistance(
                $firstPoint->latitude,
